...
SQFdata_modif$arstmade <- relevel(SQFdata_modif$arstmade, ref = "Y")
```
...
time.taken.sec - time.taken.par


```

### 4.3.4 Evaluación del error
...
pred.vect.test.sec <- foreach(j = 1:N) %dopar% {
...
for(j in 1:N){
    pred.matrix.test.sec[,j] = pred.vect.test.sec[[j]]
}

p.arstmade.test.sec = rowSums(pred.matrix.test.sec==factores[2])/N # Almacena la probabilidad
