...
```{r, warming = FALSE, message=FALSE, eval=FALSE}
prob_test_knn <- predict(model_knn_DSAMP, SQFdataTest, type="prob")[2]
pr_test_knn <- prediction(prob_test_knn, SQFdataTest$arstmade)
pred_tpr_fpr_knn <- performance(pr_test_knn, measure = "tpr", x.measure = "fpr")
plot(pred_tpr_fpr_knn, main = "ROC curve", col = "blue", lwd = 3)
abline(a = 0, b = 1, lwd = 2, lty = 2)

auc_test_knn <- performance(pr_test_knn, measure = "auc")
auc_test_knn <- auc_test_knn@y.values[[1]]
```

### 5.2.3 Random Forest

```{r, warming = FALSE, message=FALSE}
prob_test_RF <- rowSums(pred.matrix.test.par==factores[1])/N
...
confusionMatrix(factor(predict(model_glm, SQFdataTest, type="response")>0.5,
