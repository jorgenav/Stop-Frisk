...
```{r}
SQFdata_modif.dum <- SQFdata_modif %>% select(-arstmade)
```
...
### 5.2.5 Selección del número óptimo de clusters

```{r}
k_kmeans <- FitKMeans(SQFdata_modif.dum.cbind, max.clusters = 25, nstart = 25, seed = 123)
k_kmeans

NbClust(data = SQFdata_modif.dum.cbind, distance = 'manhattan', min.nc = 2, max.nc = 10, method = 'kmeans')

PlotHartigan(k_kmeans)
```

### 5.2.6 Creación de clusters
