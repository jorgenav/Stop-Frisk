...
```{r}
# model_rf <- randomForest(x = SQFdataTrain[,-which(colnames(SQFdataTrain)=="age")], y = SQFdataTrain$arstmade, ntree=100, 
#                          importance=FALSE, proximity=TRUE, mtry=4, replace=FALSE)
...
model_rf_10 <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
    ...
    randomForest(arstmade ~ ., data = SQFdataTrain, ntree=ntree, mtry = 46, maxnodes = 10)
}

model_rf_25 <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
              .packages='randomForest') %dopar% {
    randomForest(arstmade ~ ., data = SQFdataTrain, ntree=ntree, mtry = 46, maxnodes = 25)
}

model_rf_50 <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
              .packages='randomForest') %dopar% {
    randomForest(arstmade ~ ., data = SQFdataTrain, ntree=ntree, mtry = 46, maxnodes = 50)
}

model_rf_75 <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
              .packages='randomForest') %dopar% {
    randomForest(arstmade ~ ., data = SQFdataTrain, ntree=ntree, mtry = 46, maxnodes = 75)
}

model_rf_120 <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
              .packages='randomForest') %dopar% {
    randomForest(arstmade ~ ., data = SQFdataTrain, ntree=ntree, mtry = 46, maxnodes = 120)
}

model_rf_200 <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
              .packages='randomForest') %dopar% {
    randomForest(arstmade ~ ., data = SQFdataTrain, ntree=ntree, mtry = 46, maxnodes = 200)
}

model_rf_NOmax <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
              .packages='randomForest') %dopar% {
    randomForest(arstmade ~ ., data = SQFdataTrain, ntree=ntree, mtry = 46)
}

model_rf_NOmax_53 <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
              .packages='randomForest') %dopar% {
    randomForest(arstmade ~ ., data = SQFdataTrain, ntree=ntree, mtry = 53)
}

# model_rf_DSAMP <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
#               .packages='randomForest') %dopar% {
#     randomForest(arstmade ~ ., data = SQFdataTrain_DSAMP, ntree=ntree)
# }
# 
# model_rf_SMOTE <- foreach(ntree=rep(100, 6), .combine=combine, .multicombine=TRUE,
#               .packages='randomForest') %dopar% {
#     randomForest(arstmade ~ ., data = SQFdataTrain_SMOTE, ntree=ntree)
# }
#==========================================================================================
...
confusionMatrix(predict(model_rforst_DSAMP, SQFdataTest), SQFdataTest$arstmade, positive="Y")
confusionMatrix(predict(model_rforst_SMOTE, SQFdataTest), SQFdataTest$arstmade, positive="Y")
...
#==========================================================================================
confusionMatrix(predict(model_rf_10, SQFdataTest), SQFdataTest$arstmade, positive="Y")$overall[1]
confusionMatrix(predict(model_rf_10, SQFdataTest), SQFdataTest$arstmade, positive="Y")$byClass
...
cM_NOmax_53 <- confusionMatrix(predict(model_rf_NOmax_53, SQFdataTest), SQFdataTest$arstmade, positive="Y")
