...
model_glm_Forw <- step(model_glm_Vacio,
                       scope = lismodel_glmt(lower = formula(model_glm_Vacio), upper = formula(model_glm)),
...
cM_glm <- confusionMatrix(factor(predict(model_glm, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")
cM_glm_Forw <- confusionMatrix(factor(predict(model_glm_Forw, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")
cM_glm_Both <- confusionMatrix(factor(predict(model_glm_Both, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")


cM_glm_DSAMP <- confusionMatrix(factor(predict(model_glm_DSAMP, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")
cM_glm_DSAMP_Forw <- confusionMatrix(factor(predict(model_glm_DSAMP_Forw, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")
cM_glm_DSAMP_Both <- confusionMatrix(factor(predict(model_glm_DSAMP_Both, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")


cM_glm_SMOTE <- confusionMatrix(factor(predict(model_glm_SMOTE, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")
cM_glm_SMOTE_Forw <- confusionMatrix(factor(predict(model_glm_SMOTE_Forw, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")
cM_glm_SMOTE_Both <- confusionMatrix(factor(predict(model_glm_SMOTE_Both, SQFdataTest)>0.5,
                                 levels=c(TRUE,FALSE),labels=c("Y","N")), SQFdataTest$arstmade, positive="Y")
...
df.comp.glm <- data.frame(glm = fun_glm_eval(model_glm),
                          # glm_Back = c(fun_glm_eval(model_glm_Back),fun_rf_eval(model_glm_Back)),
                          glm_Forw = fun_glm_eval(model_glm_Forw),
                          glm_Both = fun_glm_eval(model_glm_Both),
                          glm_DSAMP = fun_glm_eval(model_glm_DSAMP),
                          # glm_DSAMP_Back = c(fun_glm_eval(model_glm_DSAMP_Back),fun_rf_eval(glm_Forw)),
                          glm_DSAMP_Forw = fun_glm_eval(model_glm_DSAMP_Forw),
                          glm_DSAMP_Both = fun_glm_eval(model_glm_DSAMP_Both),
                          glm_SMOTE = fun_glm_eval(model_glm_SMOTE),
                          # glm_SMOTE_Back = c(fun_glm_eval(model_glm_SMOTE_Back),fun_rf_eval(glm_Forw)),
...
df.comp.glm2 <- data.frame(glm = fun_rf_eval(cM_glm),
                          # glm_Back = fun_rf_eval(model_glm_Back),
                          # glm_Forw = fun_rf_eval(model_glm_Forw),
                          # glm_Both = fun_rf_eval(model_glm_Both),
                          glm_DSAMP = fun_rf_eval(cM_glm_DSAMP),
                          # glm_DSAMP_Back = fun_rf_eval(glm_Forw),
                          glm_DSAMP_Forw = fun_rf_eval(cM_glm_DSAMP_Forw),
                          glm_DSAMP_Both = fun_rf_eval(cM_glm_DSAMP_Both),
                          glm_SMOTE = fun_rf_eval(cM_glm_SMOTE),
                          # glm_SMOTE_Back = fun_rf_eval(glm_Forw),
                          # glm_SMOTE_Forw = fun_rf_eval(model_glm_SMOTE_Forw),
                          # glm_SMOTE_Both = fun_rf_eval(model_glm_SMOTE_Both),
