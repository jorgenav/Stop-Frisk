...
## Complexión del sospechoso (build) <- PROPUESTA CON DOS TABLAS
```{r}
...
#df <- SQFdata %>% dplyr::select(xcoord, ycoord)
#leaflet(df) %>% addTiles() %>% setView( , , zoom = 4) %>% addCircles(lat = df$xcoord, lng = df$ycoord)
```
