...
dat_modif$dob <- sapply(dat_modif$dob,myfunc_date)
...
dat_modif$searched <- factor(dat_modif$searched, levels = c(0,1), labels = c('False','True'))
dat_modif$forceuse <- factor(dat_modif$forceuse, levels = seq(1,6),
                             labels = c('defense of other','defense of self',
                                        'overcome resistence','other','suspected flight','suspected weapon'))
...
\subsection{Unión de variables}

Habría que comprobar para todos los casos si hay diferencia según cada una de las variables. Por ejemplo, si se realizan más detenciones según el tipo de arma portada o el tipo de fuerza empleada.

```{r Variables join}
# Posesión de armas
dat_modif <- dat_modif %>% 
    mutate(weap = pistol | riflshot | asltweap | knifcuti | machgun | othrweap) %>%
    select(-(pistol:othrweap))

# Fuerza física
dat_modif <- dat_modif %>% 
    mutate(phys = pf_hands | pf_wall | pf_grnd | pf_drwep | pf_ptwep | pf_baton | pf_hcuff | pf_pepsp | pf_other) %>%
    select(-(pf_hands:pf_other))

dat_modif %>% group_by(is.na(phys)) %>%
    summarise(n())

dat_modif %>% group_by(is.na(weap)) %>%
    summarise(n())
...
\subsection{Análisis exploratorio de variables}
...
dat_modif %>% filter(perstop < 40) %>%
    ggplot(mapping = aes(x = perstop, colour = city)) +
    geom_freqpoly() +
    theme_minimal() +
...
dat_modif %>% filter(perstop < 40) %>%
    ggplot(mapping = aes(x = perstop, colour = race)) +
    geom_freqpoly() +
    theme_minimal() +
    scale_x_continuous(breaks = seq(0,40,5),labels = seq(0,40,5))
...
dat_modif$age[dat_modif$age < 10 | dat_modif$age > 100] <- NA

dat_modif %>% filter(age > 79 & age < 100)

dat_modif %>% filter(dob != '1900-12-31') %>%
    select(age, dob) %>%
    mutate(a_d = 2014-as.numeric(as.character(as.Date(dob, format='%Y-%m-%d'), format='%Y'))) %>%
    filter(a_d %>% is_in(c(a_d-1,a_d+1)) & a_d > 50)
...
```{r Unknown values}
dat_modif %>% group_by(Completos = complete.cases(dat_modif)) %>%
    summarise('Nº de casos' = n())

# Find rows with too many NAs -> 0.2 = 20% de campos con NAs
length(manyNAs(dat_modif))
```
...
ggplot(aes(perstop),data=dat_modif) +
    geom_histogram() +
    facet_wrap(~race) +
    labs(title='Regresión lineal', x='Tiempo de parada', y='Frecuencia')
