...
train.numeric = train %>% select(ser_num,perobs,perstop,repcmd,revcmd,ht_feet,ht_inch,weight) %>% mutate_each(funs(scale))
test.numeric = test %>% select(ser_num,perobs,perstop,repcmd,revcmd,ht_feet,ht_inch,weight) %>% mutate_each(funs(scale))
knn_total <- knn1(train.numeric, test.numeric, train.labels)
error = sum(knn_total != test_DSAMP.labels)/length(test.labels);error
```
...
train_DSAMP.numeric = train_DSAMP %>% select(ser_num,perobs,perstop,repcmd,revcmd,ht_feet,ht_inch,weight) %>% mutate_each(funs(scale))
test_DSAMP.numeric = test_DSAMP %>% select(ser_num,perobs,perstop,repcmd,revcmd,ht_feet,ht_inch,weight) %>% mutate_each(funs(scale))
...
```{r}
#=============================================== KNN categorical DSAMP FALLA POR TAMA�O, quitar variables menos significativas, el smote y el completo tambien fallaran por disponer de mas datos
train_DSAMP.cat = SQFdataTrain_DSAMP %>%
  select(recstat,inout,trhsloc,typeofid,explnstp,othpers,arstmade,sumissue,offunif,frisked,searched,contrabn,pistol,knifcuti,othrweap,pf_hands,pf_wall,pf_grnd,pf_drwep,pf_ptwep,pf_hcuff,pf_other,radio,ac_rept,ac_inves,rf_vcrim,rf_othsw,ac_proxm,rf_attir,cs_objcs,cs_descr,cs_casng,cs_lkout,rf_vcact,cs_cloth,cs_drgtr,ac_evasv,ac_assoc,cs_furtv,rf_rfcmp,ac_cgdir,rf_verbl,cs_vcrim,cs_bulge,cs_other,ac_incid,ac_time,rf_knowl,ac_stsnd,ac_other,sb_hdobj,sb_outln,sb_admis,sb_other,rf_furt,rf_bulg,sex,race,build,city) %>% 
...
similaridadNumerica_DSAMP = sim
sim = similaridadNumerica_DSAMP
...
# KNN CARET DSAMP
```{r}
# KNN CARET SQFDATA_DSAMP
install.packages('e1071', dependencies=TRUE)
library(caret)
trctrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3)
set.seed(3333)
knn_fit_DSAMP<- train(arstmade ~., data = SQFdataTrain_DSAMP, method = "knn",
 trControl=trctrl,
 preProcess = c("center", "scale"),
 tuneLength = 10)
knn_fit_DSAMP
test_pred_DSAMP <- predict(knn_fit_DSAMP, newdata = SQFdataTrain_DSAMP)
```
# KNN CARET SMOTE
```{r}
# KNN CARET SQFDATA_DSAMP
...
trctrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3)
set.seed(3333)
knn_fit <- train(arstmade ~., data = SQFdataTrain_SMOTE, method = "knn",
 trControl=trctrl,
 preProcess = c("center", "scale"),
 tuneLength = 10)
knn_fit
test_pred <- predict(knn_fit, newdata = SQFdataTrain_SMOTE)
```

```{r}
set.seed(3333)
knn_fit_c<- train(arstmade ~., data = SQFdataTrain, method = "knn",
 trControl=trctrl,
 preProcess = c("center", "scale"),
 tuneLength = 10)
knn_fit_c
test_pred <- predict(knn_fit, newdata = SQFdataTrain)
```
