...
# Eliminaci�n de variables con 20% de NAs pero nula varianza (casos pertenecientes
...
# Eliminaci�n de variables con demasiados niveles (strings). 4 variables eliminadas
SQFdata_modif <- SQFdata_modif %>%
                    select(-c(premname, stinter, crossst, sector))



# Eliminaci�n de variables que no aparecen en el checkDataQuality. 7 variables eliminadas.
SQFdata_modif <- SQFdata_modif %>%
                    select(-c(state,zip,addrtyp,rescode,premtype,aptnum,timestop))



# Eliminaci�n de factores con m�s de 53 niveles. 4 variables eliminadas.
...
# Eliminaci�n de variables con poca varianza. 6 variables eliminadas.
SQFdata_modif <- SQFdata_modif %>%
                    select(-c(year,datestop,compyear,comppct, lineCM, dob))



# Eliminaci�n de m�s variables con poca varianza. 1 variable eliminada.
...
# Con nearZeroVar se eliminan m�s variables
...
error = sum(knn_total.numeric != test.labels)/length(test.labels);
...
knn_DSAMP.numeric <- knn1(train_DSAMP.numeric, test_DSAMP.numeric, train_DSAMP.labels)
error_DSAMP = sum(knn_DSAMP != test_DSAMP.labels)/length(test_DSAMP.labels);
...
```{r}
train_DSAMP.cat =  SQFdataTrain_DSAMP.cat %>% mutate_each(funs(as.numeric)) 
dim(train_DSAMP.cat)
SQFdataTrain.cat
...
for (i in 1:ntest) {
  ...
  prediccionCategirica_DSAMP[i] = attributes(which(z == max(z)))$names
}
...
```{r}
error2 = sum(prediccionCategirica_DSAMP != test.labels)/length(test.labels)
1-error2
...
fun_rf_eval(confusionMatrix(prediccion,test.labels))
confusionMatrix(prediccion,test.labels)$table
```
...
```{r}
test.numeric = SQFdataTest%>% select(ser_num,perobs,perstop,repcmd,revcmd,age,ht_feet,ht_inch,weight,recstat,inout,trhsloc,typeofid,othpers,arstmade,offunif,frisked,searched,pf_hands,pf_hcuff,radio,ac_rept,ac_inves,rf_vcrim,rf_othsw,ac_proxm,rf_attir,cs_descr,cs_casng,cs_lkout,rf_vcact,cs_drgtr,ac_evasv,ac_assoc,cs_furtv,rf_rfcmp,ac_cgdir,cs_vcrim,cs_bulge,cs_other,ac_incid,ac_time,rf_knowl,ac_other,sb_hdobj,sb_other,rf_furt,rf_bulg,sex,race,build,city) %>%mutate_each(funs(as.numeric))
train.numeric = SQFdataTrain%>% select(ser_num,perobs,perstop,repcmd,revcmd,age,ht_feet,ht_inch,weight,recstat,inout,trhsloc,typeofid,othpers,arstmade,offunif,frisked,searched,pf_hands,pf_hcuff,radio,ac_rept,ac_inves,rf_vcrim,rf_othsw,ac_proxm,rf_attir,cs_descr,cs_casng,cs_lkout,rf_vcact,cs_drgtr,ac_evasv,ac_assoc,cs_furtv,rf_rfcmp,ac_cgdir,cs_vcrim,cs_bulge,cs_other,ac_incid,ac_time,rf_knowl,ac_other,sb_hdobj,sb_other,rf_furt,rf_bulg,sex,race,build,city) %>%
mutate_each(funs(as.numeric))
SQFdataTest
test.numeric
trctrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3)
set.seed(3333)
knn_fit <- train(arstmade ~., data = SQFdataTrain, method = "knn",
 trControl=trctrl,
 preProcess = c("center", "scale"),
 tuneLength = 10)
knn_fit
test_pred <- predict(knn_fit, newdata = SQFdataTest)
```
Tabla de confusion variables numericas sobre el conjunto de datos completo
```{r}
fun_rf_eval(confusionMatrix(knn_total.numeric,test.labels))
confusionMatrix(knn_total.numeric,test.labels)$table
```

Tabla de confusion variables numericas sobre el conjunto de datos con downsampling
```{r}
fun_rf_eval(confusionMatrix(knn_DSAMP.numeric,test.labels))
confusionMatrix(knn_DSAMP.numeric,test.labels)$table
```

```{r}
fun_rf_eval(confusionMatrix(prediccionCategirica_DSAMP,test.labels))
```

```{r}
fun_rf_eval(confusionMatrix(test_pred,test.labels))
confusionMatrix(test_pred,test.labels)$table
```